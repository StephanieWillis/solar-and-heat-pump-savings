...
def render_outputs(solar_install: 'Solar'):
    ...
    st.write(f'We estimate that would generate {int(solar_install.generation.annual_sum_kwh):,}  kwh of electricity per year')
...
class Solar:
    ...
    def __init__(self, orientation: str, roof_height_m: float, roof_width_m: float, postcode: str):
        ...
        self.generation = usage.Consumption(profile_kwh=profile_kwh_per_m2 * self.peak_capacity_kW_out_per_kW_in_per_m2,
