...
def produce_consumption_sentence(house: 'House') -> str:
    sentence = (wrap_words_in_bold_blue_format(
                f"{int(round(house.consumption_per_fuel['electricity'].imported.annual_sum_kwh, -2)):,} kWh ")
                + "of electricity")

    if house.has_multiple_fuels:
        heat = (" and "
                + wrap_words_in_bold_blue_format(
                    f"{int(round(house.annual_consumption_per_fuel_kwh[house.heating_system.fuel.name], -2)):,} "
                    f"{house.heating_system.fuel.units}")
                + f"of {house.heating_system.fuel.name}")
        sentence += heat

    if house.solar_install.generation.overall.annual_sum_kwh != 0:
        export = (f". It would export "
                  + wrap_words_in_bold_blue_format(
                    f"{int(round(house.consumption_per_fuel['electricity'].exported.annual_sum_kwh, -2)):,} kWh ")
                  + "of the"
                  + wrap_words_in_bold_blue_format(
                    f" {int(round(-house.solar_install.generation.overall.annual_sum_kwh, -2))} kWh ")
                  + f"of electricity generated by your solar panels")
